...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    /** Your Code Here ... **/
    if(!is_cluster_inuse(clus))//判断簇号是否合法
        return -1;
    off_t offset; //偏移量
    offset = clus * sizeof(cluster_t);
    sector_t sec_num;
    sec_num = meta.fat_sec + offset / meta.sector_size;
    sector_read(sec_num, sector_buffer);
    offset = offset % meta.sector_size;  //求得扇区内的偏移量
    cluster_t res;
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i = 0; i < sectors_count; i++) {
        ...
        sector_read(from_sector + i, buffer);  //读扇区
        for(size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE) {
            ...
            if (is_valid(dir) && check_name(name, len, dir)) //目录合法且是待查找的目录项
            {
                slot->dir = *dir;
                slot->offset = off;
            ...
            else if (is_free(dir)) //如果该目录项为空,将该目录项的信息填入slot
            {
                slot->dir = *dir;
                slot->offset = off;
                slot->sector = from_sector + i;
 ...
 */
int find_entry_internal(const char* path, DirEntrySlot* slot, const char** remains) {  //依次读取路径每一级目录的内容,找到path所对应的目录项
    *remains = path;
    *remains += strspn(*remains, "/");    // 跳过开头的'/',向后移位
    ...
    while (**remains != '\0' && state == FIND_EXIST) {
        ...
        if(level == 0) {
            ...
            sector_t root_sec;
            size_t nsec;
            root_sec = meta.root_sec;  //根目录扇区号
            nsec = meta.root_sectors;  //根目录扇区数
            ...
            while (is_cluster_inuse(clus)) {    // 依次查找每个簇
                ...
                sector_t first_sec;
                first_sec = cluster_first_sector(clus); //找到该簇的第一个扇区
                state = find_entry_in_sectors(*remains, len, first_sec, meta.sec_per_clus, slot);//查找目录项
                if(state < 0) { // 出现错误
                    return state;
        ...
        next_level += strspn(next_level, "/");  //消除'/'
...
int fat16_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, 
    ...
    if(!root) {  //不是根目录
        ...
        clus = dir->DIR_FstClusLO;    // 不是根目录,找到文件的首簇号
    ...
    while (root || is_cluster_inuse(clus)) {
        ...
        for(size_t i = 0; i < nsec; i++) {   //遍历每个扇区
            ...
            for (size_t offset_sec = 0; offset_sec < meta.sector_size; offset_sec += DIR_ENTRY_SIZE)  //遍历扇区里面的每个目录条目
            {
                DIR_ENTRY* dir = (DIR_ENTRY*)(sector_buffer + offset_sec);//目录条目
                if (is_valid(dir))//合法
                {
                    to_longname(dir->DIR_Name, name, FAT_NAME_LEN);
                    filler(buf, name, NULL, 0, 0);
                }
                else if ( is_free(dir))  //不合法且为空项
                {
                    break;
                }
            }
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    if(is_directory(dir->DIR_Attr)) {  //是目录项
        return -EISDIR;
    }
    if(offset > dir->DIR_FileSize) {  //偏移量大于文件大小
        return -EINVAL;
    }
    ...
    //int read_from_cluster_at_offset(cluster_t clus, off_t offset, char* data, size_t size) 
    char clus_buffer[meta.cluster_size];  //簇数据缓存区
    size_t clus_off = offset % meta.cluster_size;  //簇内的偏移
    clus = clus + offset / meta.cluster_size;  //要读的簇号
    while (p < size)
    {
        size_t len = min(size - p, meta.cluster_size - clus_off);  //读取的长度是当前未读取的数据或者该簇剩余大小的最小值
        int ret = read_from_cluster_at_offset(clus, clus_off, clus_buffer, len);
        if (ret < 0)
        {
            return ret;
        }
        memcpy(buffer + p, clus_buffer + clus_off, len);
        p = p + len;
        clus_off = 0;
        clus = read_fat_entry(clus);  //找到下一个要读取的簇号
    }
    return p;
}
...
int dir_entry_write(DirEntrySlot slot) {
    ...
    //  3. 将整个扇区完整写回
    sector_read(slot.sector, sector_buffer);  //读对应的扇区
    memcpy(slot.offset + sector_buffer, &slot.dir, DIR_ENTRY_SIZE);  //将目录项写入对应的位置
    sector_write(slot.sector, sector_buffer);  //将整个扇区写回
    return 0;
}
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    size_t sec_off = clus_off % meta.sector_size; //扇区内偏移量
    for(size_t i = 0; i < meta.fats; i++) {
        ...
        //   2. 读取该扇区并在对应位置写入数据
        //   3. 将该扇区写回
        sector_t fat_start_sec = meta.fat_sec + i * meta.sec_per_fat;  //第i个fat表开始的扇区
        sector_t fat_entry_sec = fat_start_sec + clus_sec;  //要写的fat表项所在扇区
        sector_read(fat_entry_sec, sector_buffer);  //读取该扇区
        memcpy(sector_buffer + sec_off, &data, sizeof(cluster_t));
        sector_write(fat_entry_sec, sector_buffer);   
    }
    return 0;
}
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    cluster_t scan_num = CLUSTER_MIN; //fat表的前两项被系统占用
    cluster_t fat_buf;
    for (scan_num = CLUSTER_MIN; scan_num < CLUSTER_MAX && allocated < n; scan_num++)
    {
        fat_buf = read_fat_entry(scan_num);
        if (fat_buf == CLUSTER_FREE)  //找到空闲簇
        {
            clusters[allocated] = scan_num;
            allocated++;
        }
    }
    ...
    for (size_t i = 0; i < n; i++)
    {
        write_fat_entry(clusters[i], clusters[i + 1]);  //将每个簇通过FAT表项指向下一个簇
    }
    // write_fat_entry(clusters[n], CLUSTER_END);
...
int fat16_unlink(const char *path) {
    ...
    DirEntrySlot slot;
    DIR_ENTRY* dir = &(slot.dir);
    int ret = find_entry(path, &slot);  //找到对应的slot
    if(ret < 0) {
        return ret;
    }
    if(is_directory(dir->DIR_Attr)) {
    ...
    ret = free_clusters(dir->DIR_FstClusLO);//释放该文件所在的簇
    if(ret < 0) {
        return ret;
    }
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    // TODO: 2.6: 参考fat16_mknod实现，创建新目录
    // Hint: 注意设置的属性不同。
    ...
    int ret;
    ret = find_empty_slot(path, &slot, &dirname);//找到一个空槽,并检查是否有重名
    if (ret < 0)
    {
        return ret;
    }
    char shortname[11];
    ret = to_shortname(dirname, MAX_NAME_LEN, shortname);  //转换成短文件名
    if (ret < 0)
    {
        return ret;
    }
    ...
    ret = dir_entry_create(slot, shortname, ATTR_DIRECTORY, first_clus, 0);//创建目录项
    if (ret < 0)
    {
        return ret;
    }
    ...
    // Hint: 记得修改下面的返回值。
    DirEntrySlot dir_slot0, dir_slot1;//分别存放.和..
    char shortname0[11];
    char shortname1[11];
    dir_slot0.sector = cluster_first_sector(first_clus);
    dir_slot0.offset = 0;
    dir_slot1.sector = cluster_first_sector(first_clus);//写在同一个扇区中
    dir_slot1.offset += DIR_ENTRY_SIZE;
    ret = to_shortname(DOT_NAME, 1, shortname0);
    if (ret < 0)
    {
        return ret;
    }
    ret = to_shortname(DOTDOT_NAME, 2, shortname1);
    if (ret < 0)
    {
        return ret;
    }
    ret = dir_entry_create(dir_slot0, shortname0, ATTR_DIRECTORY, 0, 0);//创建子目录
    if (ret < 0)
    {
        return ret;
    }
    ret = dir_entry_create(dir_slot0, shortname1, ATTR_DIRECTORY, 0, 0);//创建子目录
    if (ret < 0)
    {
        return ret;
    }
...
int fat16_rmdir(const char *path) {
    ...
    // Hint: 记得修改下面的返回值
    char sector_buffer[MAX_LOGICAL_SECTOR_SIZE];
    cluster_t clus = dir->DIR_FstClusLO;//起始簇号
    while (is_cluster_inuse(clus))
    {
        sector_t first_sector = cluster_first_sector(clus);  //找到clus的第一个扇区
        for (size_t i = 0; i < meta.sec_per_clus; i++)  //遍历所有的扇区
        {
            sector_read(first_sector + i, sector_buffer);
            for (size_t offset = 0; offset < meta.sector_size; offset += DIR_ENTRY_SIZE)
            {
                DIR_ENTRY *dir_i = (DIR_ENTRY*)(sector_buffer + offset);
                if (!is_free(dir_i))  //目录条目非空
                {
                    if (is_dot(dir_i))  //如果是.或者..,继续
                    {
                        continue;
                    }
                    else if (!is_deleted(dir_i))
                    {
                        return -ENOTEMPTY;  //不为空
                    }
                }
                else   //空目录直接跳出这轮循环
                    break;
        ...
        clus = read_fat_entry(clus);//找到下一个表项
    }
    ret = free_clusters(dir->DIR_FstClusLO); //标记释放第一个簇
    if (ret < 0)
    {
        return ret;
    }
    dir->DIR_Name[0] = NAME_DELETED; //已经删除
    ret = dir_entry_write(slot);
    if (ret < 0)
    {
        return ret;
...
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    ...
    uint32_t sec = cluster_first_sector(clus) + offset / meta.sector_size;  //找到对应的扇区
    size_t sec_off = offset % meta.sector_size;  //得到扇区内偏移
    while (pos < size)
    {
        int ret = sector_read(sec, sector_buffer);  //读扇区数据到buffer中
        if (ret < 0)
        {
            return pos;
        }
        size_t len = min(size - pos, meta.sector_size - sec_off);  //当前扇区剩余位置和剩余字节数的最小值
        memcpy(sector_buffer + sec_off, data + pos, len);
        sector_write(sec, sector_buffer);  //将数据写入扇区
        pos = pos + len;
        sec_off = 0;
        sec++;
    }
...
int file_reserve_clusters(DIR_ENTRY* dir, size_t size) {
    ...
    size_t clus_need_nums = size / meta.cluster_size + ((size % meta.cluster_size == 0) ? 0 : 1);  //如果不是恰好整数个簇,需要多加一个
    if (dir->DIR_FstClusLO == 0)  //文件没有簇
    {
        int ret = alloc_clusters(clus_need_nums, &dir->DIR_FstClusLO);
        if (ret < 0)
        {
            return ret;
        }
    ...
    {
        cluster_t clus = dir->DIR_FstClusLO; //文件的第一个簇
        cluster_t next = read_fat_entry(clus);  //读下一个簇的簇号
        size_t clus_cnt = 1;
        while (is_cluster_inuse(next))  //遍历得到当前已有的簇数
        {
            clus = next;
            clus_cnt++;
            next = read_fat_entry(clus);
        }
        size_t clus_to_allocated = clus_need_nums - clus_cnt;  //还需分配的簇数
        if (clus_to_allocated > 0)
        {
            cluster_t first_alloc_clus; //第一个分配的clus
            int ret = alloc_clusters(clus_to_allocated, &first_alloc_clus);
            if (ret < 0)
            {
                return ret;
            }
            ret = write_fat_entry(clus, first_alloc_clus);  //拼接
            if (ret < 0)
            {
                return ret;
            }
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    DirEntrySlot slot;
    int ret;
    ret = find_entry(path, &slot);   //找到path对应的目录项
    if (ret < 0)
    {
        return ret;
    }
    DIR_ENTRY* dir = &(slot.dir);
    if (dir->DIR_FileSize < offset + size)//当前文件大小不够,扩充至足够的大小
    {
        ret = file_reserve_clusters(dir, size + offset);
        if (ret < 0)
        {
            return ret;
        }
    ...
    cluster_t clus = dir->DIR_FstClusLO; 
    cluster_t clus_num = offset / meta.cluster_size; //offset所在的簇数
    off_t clus_offset = offset % meta.cluster_size;  //簇内的偏移
    while (clus_num) //遍历得到offset所在簇的簇号
    {
        clus = read_fat_entry(clus); 
        clus_num--;
    }
    ...
    while (pos < size)
    {
        size_t len = min(size - pos, meta.cluster_size - clus_offset);  //写入该簇的字节数
        ret = write_to_cluster_at_offset(clus, clus_offset, data + pos, len);  //写入簇区
        if (ret != len)  //写入失败
        {
            return -ENOTSUP;
        }
...
int fat16_truncate(const char *path, off_t size, struct fuse_file_info* fi) {
    ...
    DIR_ENTRY* dir = &(slot.dir);
    int ret;
    ret = find_entry(path, &slot);
    if (ret < 0)
    {
        return ret;
    }
    cluster_t clus_num1 = dir->DIR_FileSize / meta.cluster_size + (dir->DIR_FileSize % meta.cluster_size == 0 ? 0 : 1);
    cluster_t clus_num2 = size / meta.cluster_size + (size % meta.cluster_size == 0 ? 0 : 1);
    if (clus_num1 < clus_num2)  //原有文件簇数比新的小,需要拓展
    {
        ret = file_reserve_clusters(dir, size);  //补的簇内容全为0x00,已经修改过文件大小了
        if (ret < 0)
        {
            return ret;
        }
    }
    else if (clus_num1 > clus_num2)  //原有文件簇数比新的大,需要截断
    {
        cluster_t clus_cnt = 1;
        cluster_t clus = dir->DIR_FstClusLO;  //从第一个簇开始遍历,直到第clus_num2个簇
        while (clus_cnt < clus_num2)
        {
            clus = read_fat_entry(clus);
            clus_cnt++;
        }  //此时clus是第clus_num2个簇的簇号
        cluster_t clus_to_free = read_fat_entry(clus);  //第clus_num2 + 1个簇的簇号
        write_fat_entry(clus, CLUSTER_END);  //最后一个簇的fat表项写入文件结束的标志
        ret = free_clusters(clus_to_free);
        if (ret < 0)
        {
            return ret;
        }
        dir->DIR_FileSize = size;//直接修改文件大小即可
    }
    else
        dir->DIR_FileSize = size;
    dir_entry_write(slot);
