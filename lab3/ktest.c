...
// 定义要输出的文件
#define OUTPUT_FILE "/home/vboxuser/code/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff859092a0;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff858ea8a0;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        struct vm_area_struct *vma = mm->mmap; //VMA链表
        while (vma != NULL)
        {
            vma = vma->vm_next;//遍历链表
...
static void print_mm_active_info(void)
{
    ...
    if (mm)
    {
        ...
        while (vma != NULL)//遍历链表
        {
            unsigned long virt_addr = vma->vm_start; //VMA开始的地址
            unsigned long virt_addr_end = vma->vm_end; //VMA结束的地址
            while (virt_addr < virt_addr_end) //对每个vma,求所有的页
            {
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                if (!IS_ERR_OR_NULL(page)) //页面非空且有效
                {
                    unsigned long vm_flags;
                    int freq = mpage_referenced(page, 0, (struct mem_cgroup *)(page->memcg_data) ,&vm_flags);
                    if (freq > 0)//最近被访问过
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)  //将虚拟地址转换成物理页
{
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        struct vm_area_struct *vma = mm->mmap;
        while (vma != NULL)  //遍历vma
        {
            unsigned long virt_addr = vma->vm_start; //vma的起始地址
            unsigned long virt_addr_end = vma->vm_end;  //vma的截止地址
            while (virt_addr < virt_addr_end)
            {
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
                if (!IS_ERR_OR_NULL(page)) //非空且有效
                {
                    unsigned long phy_addr = virt2phys(mm, virt_addr);
                    unsigned long tmp1 = (unsigned long)page_to_pfn(page);
                    if (tmp1 != phy_addr)
                    {
                        record_two_data(virt_addr, (unsigned long)page_to_pfn(page));
                    }
                    record_two_data(virt_addr, phy_addr);//写入虚拟地址和页面物理号
                }
                virt_addr += PAGE_SIZE;               
            }
            vma = vma->vm_next;
...
static void print_seg_info(void)
{
    ...
    if (ktest_func == 4) //func4
    {
        start = mm->start_data;
        addr_end = mm->end_data; //数据段截止的地址
    }
    else if (ktest_func == 5) //func5
    {
        start = mm->start_code;
        addr_end = mm->end_code; //代码段截止的地址
    }
    
    addr = start;//段开始的地址
    struct vm_area_struct *vma;
    struct vm_area_struct *vma_last = NULL; //标记上一次访问的vma
    while (addr <= addr_end)//遍历所有数据段的地址
    {
        vma = find_vma(mm, addr);
        if (vma != vma_last) //vma不相同则输出该vma对应的内容
        {
            unsigned long virt_addr = vma->vm_start;
            while (virt_addr <= vma->vm_end)
            {
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);  //得到页描述符
                if (!IS_ERR_OR_NULL(page)) //确保页非空且有效
                {
                    char* vaddr = kmap_atomic(page);  //可以直接访问的虚拟地址
                    if (virt_addr < start && ((virt_addr + PAGE_SIZE) <= addr_end)) //起始部分在数据段外
                    {
                        memcpy(buf, vaddr - virt_addr + start, virt_addr + PAGE_SIZE - start);
                        curr_buf_length += (virt_addr + PAGE_SIZE - start);
                    }
                    else if (virt_addr >= start && (virt_addr + PAGE_SIZE) > addr_end) //结尾部分在数据段外
                    {
                        memcpy(buf, vaddr, addr_end - virt_addr);
                        curr_buf_length += (addr_end - virt_addr);
                    }
                    else if (virt_addr < start && ((virt_addr + PAGE_SIZE) > addr_end)) //头尾都在外面
                    {
                        memcpy(buf, vaddr + start - virt_addr, addr_end - start);
                        curr_buf_length += (addr_end - start);                            
                    }
                    else if (virt_addr >= start && ((virt_addr + PAGE_SIZE) <= addr_end)) //都在数据段内
                    {
